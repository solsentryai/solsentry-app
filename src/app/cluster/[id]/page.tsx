    ...
    <>
      ...
      <main>
        <PageHeader
          ...
          title={<span style={{ fontFamily: "var(--font-mono)", fontSize: 36 }}>{cluster.cluster_id}</span>}
        ...
        >
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 8 }}>
            <span className={`risk-badge ${riskLevel}`} style={{ padding: "6px 14px", fontSize: 13 }}>
              {riskLevel}
            </span>
        ...
        {cluster.shared_funding_source && (
          <Section eyebrow="Funding source" title="Where the cluster's SOL came from">
            <div className="panel">
              <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                ...
                {cluster.funding_source_classification && (
                  <span className="pillar-chip track">{cluster.funding_source_classification}</span>
                )}
              </div>
              <p style={{ color: "var(--fg-2)", fontSize: 14, marginTop: 12, lineHeight: 1.6 }}>
                A shared funding source is the strongest evidence that a cluster is one actor, not many. The
                classification indicates whether the source is a known CEX (retail), a mixer (privacy or
                laundering), a bridge (cross-chain), or unknown (potentially another operator wallet).
              </p>
            </div>
          </Section>
        )}
