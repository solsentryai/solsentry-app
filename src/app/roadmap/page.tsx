...
export const metadata = {
  ...
  description: "What shipped, what's next, and what we will not build. Public roadmap, updated weekly. Feedback welcome at hello@solsentry.app.",
};
...
const GROUPS: { phase: string; subtitle: string; items: Item[] }[] = [
  {
    ...
    items: [
      ...
      { title: "4-stage scan pipeline (fast → deep → forensic → enrich)", status: "shipped" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "solsentry.app landing + full site", status: "in-progress", note: "You are reading it." },
      { title: "Self-check /me (wallet lookup)", status: "shipped" },
      { title: "Watchlist + Labels (local)", status: "shipped" },
      { title: "Glossary, Compare, Roadmap, Partners pages", status: "shipped" },
    ],
  },
  {
  ...
  },
  {
    ...
    subtitle: "May 10+",
    items: [
      { title: "LLM-backed /ask endpoint (real intent routing)", status: "later" },
      { title: "x402 gating on premium MCP tools", status: "later" },
      { title: "Contract-analysis endpoint (/v1/contract-analysis/{program})", status: "later" },
      ...
      { title: "Drain-trace pricing tier (victim: free; others: paid)", status: "later" },
    ],
  },
  {
    ...
    items: [
      { title: "Cross-chain tracking — use Range or Chainalysis", status: "wont-build" },
      { title: "Portfolio tracking — use Nansen, Zerion, DeBank", status: "wont-build" },
      { title: "Wallet app — we stay infrastructure-layer", status: "wont-build" },
      { title: "Smart-money trader copying — Nansen owns this", status: "wont-build" },
      { title: "Generic contract auditor — use Sec3, OtterSec, Neodyme", status: "wont-build" },
    ],
  },
];

const STATUS_STYLE: Record<Status, { label: string; color: string; bg: string }> = {
  shipped: { label: "shipped", color: "var(--brand-teal)", bg: "rgba(0,201,167,0.1)" },
  "in-progress": { label: "in progress", color: "var(--brand-orange)", bg: "rgba(255,107,0,0.1)" },
  next: { label: "next", color: "var(--status-warning)", bg: "rgba(255,176,32,0.1)" },
  later: { label: "later", color: "var(--fg-3)", bg: "var(--surface-2)" },
  "wont-build": { label: "won't build", color: "var(--fg-4)", bg: "transparent" },
};
      ...
      <main>
        <PageHeader
          ...
          title={
            <>
              What shipped,<br />
              what&rsquo;s <span style={{ color: "var(--brand-orange)" }}>next</span>.
            </>
          }
        ...
        >
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 8 }}>
            <a href="mailto:hello@solsentry.app?subject=SolSentry%20roadmap%20feedback" className="btn-primary">
              Send feedback →
            </a>
            <a href="https://github.com/solsentry" target="_blank" rel="noreferrer" className="btn-ghost">
              GitHub
            </a>
          </div>
        </PageHeader>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 16,
                      padding: "16px 24px",
                      borderBottom: i === g.items.length - 1 ? "none" : "1px solid var(--border-soft)",
                      alignItems: "center",
                    }}
                  >
                    <span
                    ...
                    <div>
                      <div style={{ fontSize: 15, color: "var(--fg-1)", lineHeight: 1.5 }}>{item.title}</div>
                      {item.note && (
                        <div style={{ fontSize: 12, color: "var(--fg-3)", marginTop: 4, fontStyle: "italic" }}>
                          {item.note}
                        </div>
                      )}
                    </div>
