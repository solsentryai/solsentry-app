...
export const metadata = {
  ...
  description: "Install the SolSentry MCP server in Claude Code, Cursor, or any MCP-compatible agent. Operator lookups, drain traces, and bot cluster queries — directly in your editor.",
};

const TOOLS = [
  { name: "scan_wallet", args: "wallet: string", desc: "Lookup operator profile — known flag, risk level, confirmed rugs, total tokens, rug rate, tags." },
  { name: "scan_token", args: "mint: string", desc: "Token analysis — risk, flags, dev wallet, bot cluster links, outcome if resolved." },
  { name: "top_operators", args: "limit?: number", desc: "Ranked list of the highest-risk operators by confirmed rugs and rate." },
  { name: "recent_alerts", args: "limit?: number", desc: "Latest HIGH and CRITICAL alerts with mint, dev, and flag list." },
  { name: "recent_resolutions", args: "limit?: number", desc: "Outcome stream — which predictions were validated and which missed." },
  { name: "drain_trace", args: "wallet: string", desc: "SOL flow trace, up to 10 hops, with bridge + CEX classifications." },
  { name: "network_stats", args: "—", desc: "Global counters — total scans, accuracy, runtime, operators tracked, serial ruggers identified." },
];
    ...
    <>
      ...
      <main>
        <PageHeader
          ...
          title={
            <>
              SolSentry in your <span style={{ color: "var(--brand-orange)" }}>AI agent</span>.
            </>
          }
          sub={
            <>
              Install once, query forever. Seven tools. Works in Claude Code, Cursor, Windsurf, Zed, or
              anything that speaks the Model Context Protocol. Your AI now knows every known Solana rug
              operator and can cite them by wallet.
            </>
          }
        >
        ...
        <Section eyebrow="Install" title="One command. Two config lines." id="install">
          <div className="grid-2" style={{ alignItems: "start" }}>
            <div>
              ...
              <p style={{ color: "var(--fg-2)", fontSize: 14, marginBottom: 10, lineHeight: 1.6 }}>
                Add to <code style={{ color: "var(--brand-orange)" }}>~/.claude/claude_desktop_config.json</code>{" "}
                or your project&rsquo;s <code style={{ color: "var(--brand-orange)" }}>.mcp.json</code>:
              </p>
            ...
            <div>
              ...
              <p style={{ color: "var(--fg-2)", fontSize: 14, marginBottom: 10, lineHeight: 1.6 }}>
                Add to <code style={{ color: "var(--brand-orange)" }}>~/.cursor/mcp.json</code> or workspace
                settings:
              </p>
          ...
          <div className="panel" style={{ marginTop: 24, borderLeft: "3px solid var(--brand-orange)" }}>
            <div className="label-tag" style={{ color: "var(--brand-orange)", marginBottom: 10 }}>
              First use
            </div>
            <pre style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--fg-2)", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
              {EXAMPLE_PROMPT}
            </pre>
          </div>
        </Section>
        ...
        <Section
          ...
          title="If the tool does not appear"
        >
          <ol className="step-list">
            ...
            <li>
              ...
              <code>npx</code> needs Node 18+. Run <code>npx @solsentry/mcp --version</code> in a terminal to
              confirm the binary installs.
            </li>
            <li>
              <strong>Check the API is reachable</strong>
              <code>curl https://api.solsentry.app/health</code> should return <code>200 OK</code>. If not, the
              MCP will surface a network error on first tool call.
            </li>
            <li>
